#!/usr/bin/env python3
"""
Test the latest TextGrad+GEPA-optimized prompt against all observations.
This script loads the most recent TextGrad+GEPA optimization result (TextGrad refinement of GEPA prompt),
extracts the optimized prompt, and tests it against all observations using OpenAI GPT-4o-mini.
"""
...
def find_latest_textgrad_gepa_result() -> Optional[Path]:
    """Find the most recent TextGrad+GEPA optimization result directory."""
    ...
    # Find all TextGrad+GEPA result directories
    textgrad_gepa_dirs = [d for d in results_dir.iterdir() if d.is_dir() and d.name.startswith("textgrad_gepa_context_compression_")]
    
    if not textgrad_gepa_dirs:
        logger.error("No TextGrad+GEPA optimization results found")
        return None
    
    # Sort by creation time and get the latest
    latest_dir = max(textgrad_gepa_dirs, key=lambda d: d.stat().st_mtime)
    logger.info(f"Found latest TextGrad+GEPA result: {latest_dir}")
    return latest_dir

def load_optimized_prompt(textgrad_gepa_dir: Path) -> Optional[str]:
    """Load the optimized prompt from the TextGrad+GEPA result."""
    # First try to load from the experiment results JSON
    experiment_file = textgrad_gepa_dir / "experiment_results.json"
    ...
    if experiment_file.exists():
        try:
            ...
            if optimized_prompt:
                logger.info(f"Loaded TextGrad+GEPA optimized prompt from experiment results ({len(optimized_prompt)} chars)")
                logger.info(f"Prompt preview: {optimized_prompt[:200]}...")
                
                # Also log information about the GEPA source
                gepa_source = experiment_data.get("gepa_source", "unknown")
                logger.info(f"Original GEPA source: {Path(gepa_source).name if gepa_source != 'unknown' else 'unknown'}")
    ...
    prompt_file = textgrad_gepa_dir / "optimized_prompt.txt"
    ...
    try:
        ...
        logger.info(f"Loaded TextGrad+GEPA optimized prompt from text file ({len(optimized_prompt)} chars)")
        logger.info(f"Prompt preview: {optimized_prompt[:200]}...")
        return optimized_prompt
...
def main():
    logger.info("Testing latest TextGrad+GEPA-optimized prompt")
    logger.info("=" * 50)
    ...
    # Find latest TextGrad+GEPA result
    logger.info("1. Finding latest TextGrad+GEPA optimization result...")
    latest_textgrad_gepa_dir = find_latest_textgrad_gepa_result()
    if not latest_textgrad_gepa_dir:
        return 1
    
    # Load optimized prompt
    logger.info("2. Loading TextGrad+GEPA optimized prompt...")
    optimized_instructions = load_optimized_prompt(latest_textgrad_gepa_dir)
    ...
    test_dir = Path(__file__).parent.parent / "data" / "tests" / f"textgrad-gepa-gpt-4o-mini-test-{timestamp}"
    ...
    config = {
        "timestamp": timestamp,
        "textgrad_gepa_source": str(latest_textgrad_gepa_dir),
        "optimized_instructions": optimized_instructions,
        "total_observations": len(observations),
        "model": "gpt-4o-mini-2024-07-18",
        "optimization_method": "TextGrad TGD on GEPA"
    }
    ...
    logger.info(f"5. Testing {len(observations)} observations with TextGrad+GEPA-optimized prompt...")
    results = []
    successful_tests = 0
    ...
    # Log results
    logger.info("=" * 50)
    logger.info("TEXTGRAD+GEPA TEST RESULTS SUMMARY")
    logger.info("=" * 50)
