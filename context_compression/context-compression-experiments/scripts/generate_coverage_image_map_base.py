...
def generate_coverage_map(test_folder: Path, output_path: str = "coverage_map.png", color: str = "red"):
    ...
    gpt4o_folder = Path(__file__).parent.parent / "data" / "gpt-4o"
    if not gpt4o_folder.is_dir():
        print(f"Error: 'gpt-4o' folder not found at {gpt4o_folder}")
        return
    ...
    for json_file in json_files:
        ...
        # Load corresponding gpt-4o file for gold output
        gpt4o_file = gpt4o_folder / f"{observation_id}.json"
        gold_output = ""
        if gpt4o_file.exists():
            with open(gpt4o_file, 'r') as f:
                gpt4o_data = json.load(f)
                gold_output = gpt4o_data.get("output", "")
        ...
        gold_result = match_lines(context, gold_output)

        all_results.append(match_result)
        max_lines = max(max_lines, len(match_result['context_lines']))
        all_gold_results.append(gold_result)
    ...
    for row, result in enumerate(all_results):
        ...
        gold_lines = all_gold_results[row]['context_lines']
        gold_overlap_lines = all_gold_results[row]['overlap_line_numbers']

        # paint the gold lines in blue
        for i in range(len(gold_lines)):
            pixel_color = (0, 0, 255) if i in gold_overlap_lines else (50, 50, 50) # Dark grey for non-overlap
        ...
        for i in range(len(context_lines)):
            ...
            for x in range(line_width):
                for y in range(line_height):
                    current_pixel = pixels[i * line_width + x, row * line_height + y]
                    if current_pixel == (0, 0, 255):  # Blue color
                        # If pixel is blue and about to be painted coverage_color, make it white
                        if i in overlap_lines:
                            pixels[i * line_width + x, row * line_height + y] = (255, 255, 255)  # White
                        # Otherwise keep it blue (don't change it)
                    else:
                        pixels[i * line_width + x, row * line_height + y] = pixel_color
